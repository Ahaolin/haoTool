...
public class ExtensionLoader<T> {
    ...
    private static final String DUBBO_DIRECTORY = "META-INF/custom/";
    ...
    // cusIgnore 运行中查看代码使用的
    private static final Map<Class<?>,String> CODE_MAP = new ConcurrentHashMap<>();
    ...
    public List<T> getActivateExtension(URL url, String key, String group) {
        ...
        // 获得符合自动激活条件的拓展对象数组
        return getActivateExtension(url, value == null || value.length() == 0 ? null : CommonConstants.COMMA_SPLIT_PATTERN.split(value), group);
    }
    ...
    public List<T> getActivateExtension(URL url, String[] values, String group) {
        ...
        if (!names.contains(CommonConstants.REMOVE_VALUE_PREFIX + CommonConstants.DEFAULT_KEY)) { // 如果 `cachedActivates`的 key在names中，此处不处理，下面处理。
            // 获得拓展实现类数组
            getExtensionClasses();
            // 循环
            for (Map.Entry<String, Activate> entry : cachedActivates.entrySet()) {
                ...
                if (isMatchGroup(group, activate.group())) { // 匹配分组
                    ...
                    if (!names.contains(name) // 不包含在自定义配置里。如果包含，会在下面的代码处理。
                            && !names.contains(CommonConstants.REMOVE_VALUE_PREFIX + name) // 判断是否配置移除。例如 <dubbo:service filter="-monitor" />，则 MonitorFilter 会被移除
        ...
        for (int i = 0; i < names.size(); i++) {
            ...
            if (!name.startsWith(CommonConstants.REMOVE_VALUE_PREFIX) && !names.contains(CommonConstants.REMOVE_VALUE_PREFIX + name)) { // 判断非移除的
    ...
    private Class<?> createAdaptiveExtensionClass() {
        // 自动生成自适应拓展的代码实现的字符串
        String code = createAdaptiveExtensionClassCode();  CODE_MAP.put(type,code);
        // 编译代码，并返回该类
        ClassLoader classLoader = findClassLoader();
    ...
    private String createAdaptiveExtensionClassCode() {
        ...
        for (Method method : methods) {
            ...
            // @Adaptive 注解，生成方法体的代码
                ...
                /**
                 *  有 protocol   使用 `URL#getProtocl()` 方法。
                 *  否则          使用`URL#getParameter()` 方法。
                 */
                // 默认拓展名
